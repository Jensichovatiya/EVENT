...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO Tracket_Master_Booking_Attendee (BookingId, FullName, Email, MobileNo, SeatNo, IsDeleted, CreatedBy, CreatedDate, CreatedFrom)
        SELECT @BookingId, AttendeeName, Email, PhoneNumber, SeatNo, 0, @CreatedBy, GETDATE(), @CreatedFrom
        FROM (
            SELECT 
        ...
        UPDATE S
        SET S.IsBooked = CASE WHEN @BookingStatus = 1 THEN 1 ELSE 0 END,
            S.SeatStatus = CASE WHEN @BookingStatus = 1 THEN 'Booked' ELSE 'Hold' END,
            S.BookingId = @BookingId
        FROM Tracket_Master_Event_Zone_Seat S
        LEFT JOIN Tracket_Master_Event_Zone_Asset A ON S.ZoneAssetId = A.ZoneAssetId
        LEFT JOIN Tracket_Master_Asset AST ON A.AssetId = AST.AssetId
        WHERE S.EventId = @EventId AND S.ZoneId = @ZoneId
          AND (S.SeatNumber + ISNULL(' (' + AST.AssetName + ')', '')) IN (
        ...
        -- If it is a Group Ticket (TicketCategoryId = 74), insert into Tracket_Master_Group_Booking
        IF @BookingTypeId = 1 AND @TicketTypeId IS NOT NULL
        BEGIN
            IF EXISTS (
                SELECT 1 FROM [EVENT_Master].[dbo].[Tracket_Master_Event_Ticket]
                WHERE TicketId = @TicketTypeId AND TicketCategoryId = 74 AND IsDeleted = 0
            )
            BEGIN
                DECLARE @GroupName NVARCHAR(200);
                -- Get the first attendee's name or buyer name + " Group"
                SELECT TOP 1 @GroupName = ISNULL(AttendeeName, 'Group Booking') + ' Group'
                FROM (
                    SELECT ISNULL(AttendeeName_P, AttendeeName_C) AS AttendeeName
                    FROM OPENJSON(@JsonData, '$.Attendees')
                    WITH (
                        AttendeeName_P NVARCHAR(100) '$.AttendeeName',
                        AttendeeName_C NVARCHAR(100) '$.attendeeName'
                ...
                ) A WHERE AttendeeName IS NOT NULL AND AttendeeName <> '';

                IF @GroupName IS NULL SET @GroupName = 'Group Booking ' + @BookingNo;
