...
export const BookingPassesPage: React.FC = () => {
    ...
    const fetchPasses = async () => {
      ...
      try {
        ...
        if (res.success && res.data) {
          const filtered = res.data.filter((p: any) => p.bookingId === Number(bookingId));
          setPasses(filtered);
        }
      } catch (err) {
        console.error(err);
  ...
  const handleDownloadSingle = async (pass: any) => {
    const elementId = `ticket-container-${pass.passCode}`;
    ...
    setDownloadingSingle(pass.passCode);
    try {
    ...
    try {
      for (let i = 0; i < passes.length; i++) {
        ...
        if (element) {
          ...
          const link = document.createElement('a');
          link.href = image;
          link.download = `Pass_${i + 1}_${pass.eventName.replace(/\s+/g, '_')}_Seat_${pass.seatNo || 'NA'}_${pass.passCode}.png`;
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
      ...
      <Container maxWidth="md">
        ...
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2, mb: 5 }}>
          <Box>
            ...
            <Typography variant="body2" color="textSecondary">
              Booking Ref: <strong style={{ color: '#3b82f6' }}>{firstPass.passCode.split('-')[0] || bookingId}</strong> | Event: <strong>{firstPass.eventName}</strong>
            </Typography>
          </Box>
        ...
        <Grid container spacing={4} sx={{ justifyContent: 'center' }}>
          {passes.map((pass, index) => (
            <Grid item xs={12} key={pass.passId || index} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <Card sx={{ p: 0.5, borderRadius: 4, boxShadow: '0 10px 30px rgba(0,0,0,0.04)', bgcolor: '#fff', border: '1px solid #e2e8f0', width: 'fit-content' }}>
                <div id={`ticket-container-${pass.passCode}`} className="ticket-container">
                  ...
                  <div className="ticket-right">
                    <div className="ticket-right-content">
                      <div className="ticket-event-name">{pass.eventName}</div>
                      <div className="ticket-date-display">
                        {new Date(pass.slotDate).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })}
                      </div>
                      <div className="ticket-time-display">
                        DOORS WILL OPEN AT {pass.startTime ? pass.startTime.substring(0, 5) : '00:00'}
                      </div>

                      {(pass.venueName || pass.addressLine1) && (
                        <div className="ticket-location-display">
                          <strong>VENUE:</strong> {pass.venueName || 'N/A'}
                          {(pass.addressLine1 || pass.addressLine2 || pass.city) && (
                            <div className="ticket-location-address">
                              {[pass.addressLine1, pass.addressLine2, pass.city, pass.state, pass.country].filter(Boolean).join(', ')}
                              {pass.googleMapLink && (
                                <a href={pass.googleMapLink} target="_blank" rel="noopener noreferrer" className="ticket-map-link">
                                  (View Map)
                                </a>
                              )}
                            </div>
                      ...
                      <div className="ticket-details-table">
                        <div className="details-column">
                          ...
                          <span className="detail-value">{pass.seatNo || 'N/A'}</span>
                        </div>
                        <div className="details-divider"></div>
                        <div className="details-column">
                          <span className="detail-label">ZONE</span>
                          <span className="detail-value">{pass.zoneName || 'General'}</span>
                        </div>
                        <div className="details-divider"></div>
                        <div className="details-column">
                          <span className="detail-label">SLOT TIME</span>
                          <span className="detail-value">
                            {pass.startTime ? pass.startTime.substring(0, 5) : '00:00'} - {pass.endTime ? pass.endTime.substring(0, 5) : 'END'}
                          </span>
                        </div>
                      </div>

                      <div className="ticket-admit-box">
                        <span className="admit-label">HOLDER: {pass.holderName.toUpperCase()}</span>
                      </div>
                    </div>
                    ...
                    <div className="ticket-qr-section">
                      <div className="qr-code-wrapper">
                        <img
                          src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(pass.passCode)}`}
                          alt="Pass QR Code"
              ...
              <Box sx={{ mt: 2, mb: 4 }}>
                <Button
                  variant="outlined"
                  startIcon={downloadingSingle === pass.passCode ? <CircularProgress size={14} /> : <Download size={15} />}
                ...
                >
                  {downloadingSingle === pass.passCode ? 'Generating Pass...' : `Download Pass (Seat: ${pass.seatNo || 'N/A'})`}
                </Button>
              </Box>
            </Grid>
          ))}
