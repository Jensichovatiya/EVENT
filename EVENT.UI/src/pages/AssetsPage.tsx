...
export const AssetsPage: React.FC = () => {
  ...
  const handleOpenEdit = (asset: any) => {
    ...
    reset({
      ...
      // Convert ISO string → Date object for the DatePicker
  ...
  const handleSaveAsset = async (data: any) => {
    try {
      ...
      // purchaseDate is a Date object from DatePicker — convert to ISO string for API
      const purchaseDateStr = data.purchaseDate instanceof Date
    ...
    <DashboardLayout>
      ...
      >
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, mt: 1 }}>
          ...
          {/* Row 4: Unit Price + Purchase Date (UI DatePicker) + Vendor */}
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr 1fr' }, gap: 2, alignItems: 'start' }}>
            <AppInput
              label="Unit Price (₹)"
              type="number"
            ...
            {/* Premium UI DatePicker for Purchase Date */}
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
              <Typography
                component="label"
                sx={{ fontSize: '0.75rem', fontWeight: 600, color: 'text.secondary', lineHeight: 1.6 }}
              >
                Purchase Date
              </Typography>
              <Controller
