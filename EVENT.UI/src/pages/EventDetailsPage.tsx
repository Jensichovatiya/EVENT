...
export const EventDetailsPage: React.FC = () => {
    ...
    const fetchAvailability = async () => {
      ...
      try {
        ...
        if (seatsRes.success && seatsRes.data && seatsRes.data.length > 0) {
          ...
          const bookedZoneSeats = seatsRes.data.filter(s => unionBookedList.includes(s.seatNumber) || s.isBooked || s.isBlocked || s.isReserved).length;
  ...
  const handleSeatClick = (seatNo: string) => {
    ...
    if (seatObj && (seatObj.isBooked || seatObj.isBlocked || seatObj.isReserved)) return;
    ...
    <DashboardLayout>
      ...
      {bookingOpen ? (
        <Box sx={{ width: '100%', mt: 2 }}>
          <BookingFlow event={event} onClose={handleCloseBooking} />
        </Box>
      ) : (
