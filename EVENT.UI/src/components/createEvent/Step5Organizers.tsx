...
import { organizerContactApi } from '../../api/organizerContactApi';
...
const PhoneField: React.FC<{ code: string; phone: string; onCode: (v: string) => void; onPhone: (v: string) => void; errorText?: string }> = ({ code, phone, onCode, onPhone, errorText }) => (
  <Box sx={{ display: 'flex', gap: 1 }}>
    ...
    <AppInput placeholder="98765 43210" value={phone} errorText={errorText} onChange={(e) => onPhone(e.target.value)} />
  </Box>
);
...
export const Step5Organizers: React.FC<StepProps> = ({ draft, onChange, errors = {} }) => {
  ...
  const lookupZip = async (zipCode: string) => {
    ...
    try {
      ...
      if (json && json.places && json.places.length > 0) {
        ...
        set({
          city: place["place name"] || o.city,
          state: place["state"] || o.state,
      ...
      if (Array.isArray(json) && json.length > 0 && json[0].Status === 'Success' && json[0].PostOffice && json[0].PostOffice.length > 0) {
        ...
        set({
          city: firstOffice.District || o.city,
          state: firstOffice.State || o.state,
          country: "India"
        });
      ...
      if (json.success && json.data && json.data.length > 0) {
        ...
        set({
          city: loc.city_name || o.city,
          state: loc.state_name || o.state,
          country: loc.country_name || o.country
        });
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const res = await organizerContactApi.getContacts();
        if (res.success && res.data) {
          const mapped = res.data.map((c) => ({
            id: c.publicId || String(c.contactId),
            name: c.contactName,
            designation: c.designation,
            role: c.roleResponsibility,
            email: c.email,
            phone: c.phone
          }));
          set({ additionalContacts: mapped });
        }
      } catch (err) {
        console.error("Failed to load organizer contacts:", err);
      }
    };
    fetchContacts();
  }, []);
  ...
  const saveContact = async () => {
    ...
    try {
      const payload = {
        publicId: (editingId && !editingId.startsWith('ct')) ? editingId : null,
        organizerId: draft.eventId || 0,
        contactName: cf.name,
        designation: cf.designation || '',
        roleResponsibility: cf.role || '',
        email: cf.email || '',
        phone: cf.phone || '',
        isActive: true,
        createdBy: 'WebUI',
      ...
      const res = await organizerContactApi.addEditContact(payload);
      if (res.success && res.data) {
        const savedContact: ContactPerson = {
          id: res.data.publicId || String(res.data.contactId),
          name: res.data.contactName,
          designation: res.data.designation,
          role: res.data.roleResponsibility,
          email: res.data.email,
          phone: res.data.phone
        };

        if (editingId) {
          set({ additionalContacts: o.additionalContacts.map((c) => (c.id === editingId ? savedContact : c)) });
        } else {
          set({ additionalContacts: [...o.additionalContacts, savedContact] });
        }
      }
    } catch (err) {
  ...
  const handleDeleteContact = async (id: string) => {
    try {
      if (id && !id.startsWith('ct')) {
        await organizerContactApi.deleteContact(id);
      }
      set({ additionalContacts: o.additionalContacts.filter((x) => x.id !== id) });
    } catch (err) {
    ...
    <StepLayout
      main={
        <>
          ...
          <Grid cols={4}>
            <Field label="Organiser Type" required><AppDropdown label="Type" options={ORGANIZER_TYPE_OPTIONS} value={o.organizerType} errorText={errors.organizerType} onChange={(e) => set({ organizerType: e.target.value as string })} /></Field>
            <Field label="Organisation / Company Name" required><AppInput placeholder="e.g. FutureX Events Pvt. Ltd." value={o.companyName} errorText={errors.companyName} onChange={(e) => set({ companyName: e.target.value })} /></Field>
          ...
          <Grid cols={4} sx={{ mt: 2.5 }}>
            ...
            <Field label="Primary Email" required><AppInput placeholder="info@example.com" value={o.primaryEmail} errorText={errors.primaryEmail} onChange={(e) => set({ primaryEmail: e.target.value })} /></Field>
            <Field label="Primary Phone" required><PhoneField code={o.primaryPhoneCode} phone={o.primaryPhone} errorText={errors.primaryPhone} onCode={(v) => set({ primaryPhoneCode: v })} onPhone={(v) => set({ primaryPhone: v })} /></Field>
          ...
          <Grid cols={4} sx={{ mt: 2.5 }}>
            <Field label="Address" required><AppInput placeholder="No. 45, 2nd Floor, MG Road" value={o.address} errorText={errors.address} onChange={(e) => set({ address: e.target.value })} /></Field>
            <Field label="City" required><AppInput placeholder="Bengaluru" value={o.city} errorText={errors.city} onChange={(e) => set({ city: e.target.value })} /></Field>
            <Field label="State" required><AppDropdown label="State" options={INDIAN_STATES} value={o.state} errorText={errors.state} onChange={(e) => set({ state: e.target.value as string })} /></Field>
            <Field label="Country" required><AppDropdown label="Country" options={COUNTRY_OPTIONS} value={o.country} errorText={errors.country} onChange={(e) => set({ country: e.target.value as string })} /></Field>
          </Grid>
            <Field label="PIN / ZIP Code" required>
              <AppInput 
                ...
                onKeyDown={(e) => { if (e.key === 'Tab') { lookupZip(o.zip); } }}
                onBlur={() => lookupZip(o.zip)}
              />
            </Field>
          ...
          <Grid cols={4}>
            <Field label="Full Name" required><AppInput placeholder="Ramesh Kumar" value={o.ownerName} errorText={errors.ownerName} onChange={(e) => set({ ownerName: e.target.value })} /></Field>
            <Field label="Designation" required><AppInput placeholder="Event Director" value={o.ownerDesignation} errorText={errors.ownerDesignation} onChange={(e) => set({ ownerDesignation: e.target.value })} /></Field>
            <Field label="Email" required><AppInput placeholder="owner@example.com" value={o.ownerEmail} errorText={errors.ownerEmail} onChange={(e) => set({ ownerEmail: e.target.value })} /></Field>
            <Field label="Phone" required><PhoneField code={o.ownerPhoneCode} phone={o.ownerPhone} errorText={errors.ownerPhone} onCode={(v) => set({ ownerPhoneCode: v })} onPhone={(v) => set({ ownerPhone: v })} /></Field>
          </Grid>
                ...
                {o.additionalContacts.length === 0 ? (
                  ...
                  <Box component="tr" key={c.id}>
                    ...
                    <Box component="td" sx={{ py: 1.1, px: 1.5, borderBottom: `1px solid ${EP.lineSoft}` }}>
                      ...
                      <IconButton size="small" onClick={() => handleDeleteContact(c.id)}><DeleteOutlineIcon fontSize="small" sx={{ color: EP.red }} /></IconButton>
                    </Box>
                  </Box>
                ))}
              </Box>
          ...
          <Grid cols={4}>
            <Field label="Contact Name" required><AppInput placeholder="Suresh Babu" value={o.emergencyName} errorText={errors.emergencyName} onChange={(e) => set({ emergencyName: e.target.value })} /></Field>
            <Field label="Relationship" required><AppInput placeholder="Admin Manager" value={o.emergencyRelationship} errorText={errors.emergencyRelationship} onChange={(e) => set({ emergencyRelationship: e.target.value })} /></Field>
            <Field label="Phone" required><PhoneField code={o.emergencyPhoneCode} phone={o.emergencyPhone} errorText={errors.emergencyPhone} onCode={(v) => set({ emergencyPhoneCode: v })} onPhone={(v) => set({ emergencyPhone: v })} /></Field>
      ...
      rail={
        <>
          ...
          <SidebarCard title="Documents">
            ...
            {DOCS.map((doc) => {
              const fileKey = doc.key === 'logo' ? 'logoFile' : doc.key === 'gst' ? 'gstCertificateFile' : doc.key === 'pan' ? 'panCardDocumentFile' : 'registrationCertificateFile';
              const fileObj = (o as any)[fileKey]?.[0];
              ...
              return (
                <Box key={doc.key} sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 0.8, borderBottom: `1px solid ${EP.lineSoft}` }}>
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    ...
                    <Typography sx={{ fontSize: '0.68rem', color: EP.faint }} noWrap>{fileName || doc.hint}</Typography>
                  </Box>
            ...
            <Box component="label" sx={{ display: 'block', border: `1px dashed ${EP.line}`, borderRadius: 1.5, p: 1.5, textAlign: 'center', mt: 1.5, cursor: 'pointer', '&:hover': { borderColor: EP.primary } }}>
              <input type="file" hidden onChange={(e) => {
                if (e.target.files && e.target.files[0]) {
                  set({ otherDocumentFile: [e.target.files[0]] });
                }
              }} />
              <AddIcon sx={{ color: EP.primary, fontSize: 18 }} />
              <Typography sx={{ fontSize: '0.78rem', fontWeight: 600, color: EP.primary }}>
                {o.otherDocumentFile?.[0]?.name ? `Other: ${o.otherDocumentFile[0].name}` : 'Upload Other Document'}
              </Typography>
