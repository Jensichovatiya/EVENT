...
export interface DateTimeInfo {
  ...
  // explicit slots for single/multi-day events
  slots: DateTimeSlot[];
}

// ---- Step 3: Venue & Location ----
export interface VenueFacilities {
  [facilityId: number]: boolean; // key = FacilityId from Tracket_Master_Event_Master_Facility
}
...
export interface ZoneLayout {
  ...
  rows: number;
  columns: number;
  arrangementType: 'Auto Arrange' | 'Manual Arrange' | '';
  assetId: number | '';
}
...
export interface EventDetailsInfo {
  ...
  columns: number;
  assetItems: AssetItem[];
  zones?: Record<number, ZoneLayout>; // map of zoneId -> layout layout cache
}
...
export interface OrganizerProfile {
  ...
  youtube: string;
  pintrest: string;
  // Organizer files for backend persistence
  logoFile?: File[];
...
export const createEmptyDraft = (): EventDraft => ({
  ...
  datetime: {
    mode: 'single', startDate: '', endDate: '', startTime: '09:00 AM', endTime: '06:00 PM',
    timezone: 'Asia/Kolkata', durationDays: '', durationHours: '', durationMinutes: '',
    allDay: false, showCountdown: true, visibilityStartDate: '', visibilityStartTime: '09:00 AM',
  ...
  details: { hallName: '', viewMode: '2d', components: [], zoneId: '', assetId: '', arrangementType: '', quantity: 50, rows: 5, columns: 5, assetItems: [] },
  organizer: {
    ...
    facebook: '', instagram: '', linkedin: '', twitter: '', youtube: '', pintrest: '',
    logoFile: [], gstCertificateFile: [], panCardDocumentFile: [], registrationCertificateFile: [], otherDocumentFile: [],
  },
  tickets: {
  ...
  media: {
    ...
    galleryFiles: [], documentFiles: [], videoFiles: [], audioFiles: [], shareTitle: '', shareDescription: '', shareImageFile: [],
  },
