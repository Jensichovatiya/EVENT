    ...
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(0,1fr) 320px' }, gap: 3, alignItems: 'start' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, position: { lg: 'sticky' }, top: 16 }}>
        ...
        <SidebarCard title="Publish Options">
          <RadioGroup value={draft.publish.publishOption} onChange={(e) => onChange('publish', { ...draft.publish, publishOption: e.target.value as any })}>
            {[['now', 'Publish Now', 'Your event will be live and visible to attendees.'], ['schedule', 'Schedule Publish', 'Choose a future date and time to publish.'], ['draft', 'Save as Draft', 'Save and continue editing later.']].map(([val, title, sub]) => (
              <FormControlLabel key={val} value={val} control={<Radio sx={{ '&.Mui-checked': { color: EP.primary } }} />}
                label={<Box sx={{ py: 0.5 }}><Typography sx={{ fontSize: '0.85rem', fontWeight: 600 }}>{title}</Typography><Typography sx={{ fontSize: '0.74rem', color: EP.faint }}>{sub}</Typography></Box>} />
            ))}
          </RadioGroup>
