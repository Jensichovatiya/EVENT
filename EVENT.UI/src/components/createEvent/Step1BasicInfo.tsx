...
export const Step1BasicInfo: React.FC<StepProps> = ({ draft, onChange, ddl, meta, errors = {} }) => {
          ...
          <Grid cols={3}>
            <Field label="Event Title" required>
              <AppInput
                ...
                onChange={(e) => set({ eventName: e.target.value, slug: data.slug || slugify(e.target.value) })}
              />
            </Field>
          ...
          <SidebarCard title="Steps Overview">
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              {WIZARD_STEPS.map((s, idx) => {
                const active = idx === meta.activeStep;
                const done = meta.completed[idx];
                return (
                  <Box key={s.key} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, py: 1 }}>
                    <Box
                      sx={{
