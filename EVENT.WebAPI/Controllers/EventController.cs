...
namespace EVENT.WebAPI.Controllers
{
    ...
    public class EventController : ControllerBase
    {
        ...
        [HttpPost("events")]
        public async Task<IActionResult> AddEditEvent([FromForm] string model)
        {
            ...
            // Validate MetaJson if present to ensure proper components and asset items layout format
            if (!string.IsNullOrEmpty(eventRequest.MetaJson))
            {
                try
                {
                    var meta = Newtonsoft.Json.JsonConvert.DeserializeObject<EventMetaRequest>(eventRequest.MetaJson);
                    if (meta == null)
                    {
                        return BadRequest(new ApiResponseModel<object> { Success = false, StatusCode = 400, Message = "Invalid metadata format." });
                    }
                }
                catch (Newtonsoft.Json.JsonException ex)
                {
                    return BadRequest(new ApiResponseModel<object> { Success = false, StatusCode = 400, Message = $"Invalid MetaJson layout structure: {ex.Message}" });
                }
            }

            var result = await _eventRepository.AddEditEvent(eventRequest, Request.Form.Files);
