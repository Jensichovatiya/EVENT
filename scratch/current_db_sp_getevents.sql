...
BEGIN
    ...
    BEGIN
        ...
        -- Table[0]: Event header + location + CurrentActiveStep
        SELECT 
            ...
            E.IsFree, E.IsPublic, E.MetaJson, EP.CurrentActiveStep,
        ...
        LEFT JOIN Tracket_Event_Progress EP ON E.EventId = EP.EventId
        ...
        -- Table[3]: Step-wise completion details from Tracket_Event_Progress
        SELECT 
            EventId,
            BasicInformationCompleted, DateTimeCompleted, VenueLocationCompleted, EventDetailsCompleted,
            OrganizersCompleted, TicketsPricingCompleted, MediaBrandingCompleted, ReviewPublishCompleted,
            CurrentActiveStep, CompletionPercentage, CreatedDate, UpdatedDate
        FROM Tracket_Event_Progress
        WHERE EventId = @ActualEventId;
    END
    ELSE
    BEGIN
        -- List all events (summary) + CurrentActiveStep
        SELECT 
            E.EventId, E.EventRId, E.PublicId,
            E.EventName, E.EventCode,
            E.EventCategoryId AS CategoryId, C.CategoryName,
            ...
            E.UserId AS OrganizerId, E.UserId,
            E.Tagline, E.Purpose, E.TargetAudience, EP.CurrentActiveStep,
            (SELECT MIN(StartDate) FROM Tracket_Master_Event_Slot WHERE EventId = E.EventId AND IsDeleted = 0) AS StartDate,
            (SELECT MAX(EndDate) FROM Tracket_Master_Event_Slot WHERE EventId = E.EventId AND IsDeleted = 0) AS EndDate,
