...
write-output "Fetching Event 20029..."
$eventRes = Invoke-RestMethod -Uri "http://localhost:5209/api/events/20029" -Method Get -Headers $headers
$event = $eventRes.data

# 3. Build EventRequest mapping
write-output "Preparing EventRequest payload..."
# Map Slots from response list to request model format
$slotsMapped = @()
if ($event.slots) {
...
[System.IO.File]::WriteAllText("scratch_payload.json", $payloadJson, [System.Text.Encoding]::UTF8)

# 4. Save Event (multipart/form-data)
