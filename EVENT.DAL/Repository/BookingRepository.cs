...
namespace EVENT.DAL.Repository
{
    public class BookingRepository : BaseRepository, IBookingRepository
    {
        ...
        public async Task<ApiResponseModel<BookingResponse>> CreateUpdateBookingAsync(BookingRequest request)
        {
            try
            {
                // Populate AddOnIdsJson so the SP can access add-on IDs as a flat JSON array string
                if (request.AddOnIds != null && request.AddOnIds.Count > 0)
                    request.AddOnIdsJson = Newtonsoft.Json.JsonConvert.SerializeObject(request.AddOnIds);
                else
                    request.AddOnIdsJson = "[]";
                    ...
                    using (var multi = await connection.QueryMultipleAsync("USP_CreateUpdateBooking", parameters, commandType: CommandType.StoredProcedure))
                    {
                        ...
                        if (status != null)
                        {
                            ...
                            if (statusCode == 200 || statusCode == 201)
                            {
                                ...
                                // Sync add-on associations if any add-ons were selected
                                if (booking != null && request.AddOnIds != null && request.AddOnIds.Count > 0)
                                {
                                    try
                                    {
                                        // Build a minimal JSON payload with the resolved BookingId and AddOnIds
                                        var addOnSyncPayload = new
                                        {
                                            BookingId = booking.BookingId,
                                            AddOnIds = request.AddOnIds,
                                            AddOnIdsJson = request.AddOnIdsJson
                                        };
                                        var syncParams = new { JsonData = JsonConvert.SerializeObject(addOnSyncPayload) };
                                        using (var syncConn = new SqlConnection(_connectionString.Value.ConnectionString))
                                        {
                                            await syncConn.OpenAsync();
                                            await syncConn.ExecuteAsync(
                                                "USP_SyncBookingAddOns",
                                                syncParams,
                                    ...
                                    {
                                        // Log but do not fail the booking if add-on sync fails
                                        System.Diagnostics.Debug.WriteLine($"[BookingRepository] AddOn sync failed: {syncEx.Message}");
                                    }
                                }
